    ...
    def consume_command(self) -> bool:
        ...
        try:
            if isinstance(do_now, cmd.TrainUnit):
                do_now.train(self)
                logger.info(f'Training  {do_now.unit.name} in {do_now.building.name}')
            elif isinstance(do_now, cmd.PlaceUnit):
                tag = do_now.place(self, self.placed_units)
                self.placed_units.add(tag)
                logger.info(f'Placing   {do_now.unit.name} at {do_now.location}')
            elif isinstance(do_now, cmd.BuildStructure):
                do_now.build(self)
                logger.info(f'Building  {do_now.unit.name} at {do_now.location}')
