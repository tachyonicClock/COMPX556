...
def test_select(population: Population):
    ...
    for i in range(len(selected._population) - 1):
        assert cmp(selected._population[i].fitness) >= cmp(
            selected._population[i + 1].fitness)


def test_stochastic_mutate(simple_population: Population):
    # IMPORTANT: This is required to get the same result every time
    random.seed(42)
    def mutator(x): return gp.Marauder()
...
def test_stochastic_sex(simple_population: Population):
    # IMPORTANT: This is required to get the same result every time
    random.seed(42)
    def sex(a, b): return gp.Quadrant([a, b, gp.Empty(), gp.Empty()])
