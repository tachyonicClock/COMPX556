...
class Tensorboard(LogCallback):
    ...
    def after_pop_eval(self, generation: int, population: Population):
        ...
        self.writer.add_scalar(
            'fitness/average', population.average_fitness_score(), generation)
        self.writer.add_scalar(
            'fitness/best', population.best_fitness_score(), generation)

        self.writer.add_scalar(
            'gas/average', population.average_gas(), generation)
        self.writer.add_scalar('gas/best', best.fitness.gas, generation)

        self.writer.add_scalar(
            'time/average', population.average_time(), generation)
        self.writer.add_scalar('time/best', best.fitness.time, generation)

        self.writer.add_scalar(
            'minerals/average', population.average_minerals(), generation)
        self.writer.add_scalar(
            'minerals/best', best.fitness.minerals, generation)

        self.writer.add_scalar(
            'structure/depth', best.genotype.depth(), generation)
        self.writer.add_scalar(
            'structure/size', best.genotype.size(), generation)
